...
class TestEntityServeTimeWindow(TestCase):
        ...
        def test_init(self):
            ...
            window = TimeWindow(recipe=self.recipe)
        ...
        def test_init_with_window(self):
            ...
            window = TimeWindow(recipe=self.recipe, window=(time(1, 0), time(2, 0)))
            # Check its the right type
            self.assertIsInstance(window, TimeWindow)
        ...
        def test_is_subset_of(self):
            ...
            window = TimeWindow(self.recipe, window=(time(1, 0), time(2, 0)))
            other = TimeWindow(self.recipe, window=(time(0, 0), time(3, 0)))
